...
![Version](https://img.shields.io/badge/release-v1.0.0-5eead4?style=flat-square)
...
Every release artifact ships with a `<asset>.sha256` sidecar. Verify with:

```bash
sha256sum -c Cove-Download-Manager-1.0.0-x86_64.AppImage.sha256
```

(or `Get-FileHash -Algorithm SHA256` on Windows). Cove Nexus's auto-update
...
4. **Delete key** — focus the downloads list, hit `Delete` to remove the
   selection (file on disk is kept; use right-click → "Remove and delete
   file" to wipe it too).
